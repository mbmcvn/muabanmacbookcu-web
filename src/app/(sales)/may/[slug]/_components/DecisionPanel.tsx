...
export function DecisionPanel({ machine }: { machine: PublicMachineDetailV1 }) {
  ...
  return <section className="detail-decision" aria-labelledby="machine-title"><div className="detail-status"><span>{formatMachineAvailability(summary.availability, summary.reservationKind)}</span>{summary.contextualLabel ? <span>{summary.contextualLabel}</span> : null}</div><h1 id="machine-title">{displayName}</h1>{configuration(machine) ? <p className="detail-configuration">{configuration(machine)}</p> : null}<p className="detail-price">{formatCurrencyVnd(summary.price)}</p>{reserved ? <p className="contact-context">Máy hiện đã được giữ. Các thao tác mua được tạm khóa.</p> : <><div className="detail-actions"><ContactActionLink id="machine-hero-contact-action" className="primary-action" />{phoneContact ? <a className="secondary-action" href={phoneContact.href} aria-label={`Gọi MBMC về ${summary.code} – ${displayName}`}>Gọi MBMC</a> : null}</div><p className="contact-context">Bạn có thể gửi: “Tôi đang quan tâm {summary.code} – {displayName}.”</p></>}</section>;
}
