...
export function MachineCard({
  ...
  return <article className="machine-card"><Link className="machine-card-link" href={withContactChannel(`/may/${machine.slug}`, channel)} aria-label={`Xem ${displayName}, ${price}`}><div className="machine-image"><MachineImage image={machine.coverImage} variant="card" fill sizes="(max-width: 639px) 38vw, (max-width: 1199px) 45vw, 350px" /></div><div className="machine-card-body"><div className="machine-card-status"><span>{formatMachineAvailability(machine.availability, machine.reservationKind)}</span>{machine.contextualLabel ? <span className="context-label">{machine.contextualLabel}</span> : null}</div><Heading>{displayName}</Heading><p className="machine-configuration">{specs}</p><p className="machine-price">{price}</p>{condition ? <p className="machine-card-condition">{condition}</p> : null}<dl className="decision-facts">{battery ? <div><dt>{battery.label}</dt><dd>{battery.value}</dd></div> : null}<div><dt>Ngoại hình</dt><dd>{machine.cosmeticGrade ?? "Chưa có dữ liệu"}</dd></div></dl><div className="machine-card-footer"><span className="machine-code">{machine.code}</span><span className="machine-card-cta">{reserved ? "Xem thông tin" : <><span className="machine-card-cta-short">Xem máy</span><span className="machine-card-cta-long">Xem chiếc máy này</span> <span aria-hidden="true">→</span></>}</span></div></div></Link></article>;
}
