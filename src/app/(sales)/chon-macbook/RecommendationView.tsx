...
import { inventoryZaloLine, type InventoryMatchViewState } from "./inventory-match-presentation";
...
export function RecommendationView({ answers, result, onRestart }: { answers: QuizAnswers; result: Recommendation; onRestart: () => void }) {
  const [inventoryState, setInventoryState] = useState<InventoryMatchViewState | null>(null);
  useEffect(() => {
    let current = true;
    loadInventoryMatches(result.profile)
      .then((state) => { if (current) setInventoryState(state); })
      .catch(() => { if (current) setInventoryState({ status: "failed" }); });
    return () => { current = false; };
  }, [result.profile]);
  const inventoryLine = inventoryState ? inventoryZaloLine(inventoryState) : null;
  const summary = [buildZaloSummary(answers, result), inventoryLine].filter(Boolean).join("\n");
  ...
  return (
    <section className="quiz-result" aria-labelledby="recommendation-title">
      <div className="result-hero">
        <div>
          ...
          <div className="result-specs">
            <div><span>RAM tối thiểu</span><strong>{profile.technical.minimumRamGb}GB</strong></div>
            <div><span>Lưu trữ</span><strong>{presentation.storageGuidance}</strong></div>
          </div>
        </div>
        <QuizIllustration
      ...
      {profile.financial.status === "conflict" && <aside className="quiz-warning"><strong>Ngân sách cần được cân lại</strong><p>Yêu cầu kỹ thuật vẫn được giữ nguyên; ngân sách chỉ được đánh giá sau khi có một máy và mức giá cụ thể.</p></aside>}
      {profile.verification.required && <aside className="quiz-warning"><strong>Kết quả này còn sơ bộ</strong><p>{profile.verification.reasons.join(" ")}</p></aside>}
      <div className="recommendation-reasons"><p className="quiz-eyebrow">Vì sao MBMC gợi ý như vậy</p><ul>{presentation.reasons.map((reason) => { const [answer, conclusion] = reason.split("\n"); return <li key={reason}><span>{answer}</span><strong>{conclusion}</strong></li>; })}</ul></div>
      <div className="recommendation-options">
        <Option title={optionTitles.bestFit} option={presentation.bestFit} />
        {presentation.alternative && <Option title={optionTitles.alternative} option={presentation.alternative} />}
        {presentation.upgrade && <Option title={optionTitles.upgrade} option={presentation.upgrade} />}
      </div>
